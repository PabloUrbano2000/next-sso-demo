...
import { MergeDto } from '@/modules/auth/dtos/merge.dto'
import { loginSocialCodeConfirm } from '@/modules/piano/services/login/login-social-code-confirm'
...
export async function POST(req: NextRequest) {
  ...
  const parsed = MergeDto.safeParse(body)
  if (!parsed.success) {
  ...
  const { email, password, confirmed_token } = body as MergeDto

  try {
    const { success: loginSuccess, error: loginError } =
      await loginSocialCodeConfirm({
        email,
        password,
        confimed_token: confirmed_token,
        api_token: apiToken,
        aid: aid
      })
